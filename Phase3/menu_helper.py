...
'''
def input_valid_key_id(employee_id, show=False):
    all_keys = get_all_keys(employee_id, show)
...
def input_valid_door(building_name, room_number, show=False):
    # List of existing doors
    all_old_doors = doors_table.find({'building name': building_name, 'room number': room_number})
    all_old_doors_names = [d['door name'] for d in all_old_doors]

    # List of possible door names
    all_new_doors_names = []
    for item in all_old_doors_names:
        if item['door name'] not in all_new_doors_names:
            all_new_doors_names.append(item['door name'])
            print(item['door name'])
